...
- 新增分页式真实任务列表接口，任务摘要来自 SQLite 且不暴露本地文件路径或完整结果；真实后端模式下任务中心改为读取该接口，并支持搜索、状态/风险筛选、失败重试和实时任务跳转。
- 新增 SQLite 态势统计接口；真实模式首页展示全量任务、活跃队列、Agent1 人工复核、Agent1+2 成功数量、24 小时趋势和最近真实任务，并明确不将其表述为四智能体报告完成。
- 新增 Agent1+Agent2 真实联调一键预检和操作手册，检查 GPU、模型依赖、四个 Agent1 权重、Qwen/LoRA 必需文件及双时相样本；在隔离临时 SQLite 中验证了无模型情况下的创建、排队、安全失败、任务列表与首页统计链路。
